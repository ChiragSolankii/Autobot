    ...
    switch(message.msgid)
    {
      case MAVLINK_MSG_ID_ROBOT_SENSOR_READINGS :
        mavlink_msg_robot_sensor_readings_decode(&message, &mav_robot_sensor_read);
	    	MotorControl.left_wheel_ticks = mav_robot_sensor_read.left_wheel_ticks;
	    	MotorControl.right_wheel_ticks= mav_robot_sensor_read.right_wheel_ticks;
        MotorControl.curr_heading = mav_robot_sensor_read.curr_heading;
        printf("left wheel ticks:[%ud] \t right wheel ticks:[%ud] \t curr heading:[%f]\n", mav_robot_sensor_read.left_wheel_ticks,MotorControl.right_wheel_ticks,mav_robot_sensor_read.curr_heading);
        break;
